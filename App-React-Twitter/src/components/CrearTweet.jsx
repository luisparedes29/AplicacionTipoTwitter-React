      ...
      {isOpen && (
        <form className="fixed flex justify-center items-center inset-0 backdrop-blur-sm  bg-black bg-opacity-30" onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end w-full ">
            ...
            <input className="mb-3 text-lg rounded-md p-1 text-azulOscuro2 font-semibold h-[30vh] w-full" type="text" value={tweet} onChange={(e)=> setTweet(e.target.value)} />

            <button type="submit" className="bg-azulGris w-[40%] text-xl p-2   rounded-lg mt-4">Publicar</button>
          </div>
        </form>
      )}
    </div>
